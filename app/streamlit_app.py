...
# -------------------------------------------------------
RISK_CONFIG = {
    "Low":      {"bg":"linear-gradient(135deg,#071a0f,#0a2415)","border":"#2ea043","text":"#3fb950","badge_bg":"#0a2415","badge_text":"#3fb950","glow":"rgba(63,185,80,0.18)","bar":"#3fb950"},
    "Moderate": {"bg":"linear-gradient(135deg,#161200,#211800)","border":"#d29922","text":"#e3b341","badge_bg":"#211800","badge_text":"#e3b341","glow":"rgba(227,179,65,0.18)","bar":"#e3b341"},
    "High":     {"bg":"linear-gradient(135deg,#221200,#2d1800)","border":"#db6d28","text":"#f0883e","badge_bg":"#2d1800","badge_text":"#f0883e","glow":"rgba(240,136,62,0.18)","bar":"#f0883e"},
    "Critical": {"bg":"linear-gradient(135deg,#220a0a,#2d0f0f)","border":"#da3633","text":"#f85149","badge_bg":"#2d0f0f","badge_text":"#f85149","glow":"rgba(248,81,73,0.22)","bar":"#f85149"},
}
RISK_ICONS = {"Low":"✓","Moderate":"◈","High":"⚠","Critical":"✕"}

POLITICAL_WORDS = [
    "trump","clinton","obama","biden","harris","modi","putin","xi jinping","macron",
    "zelensky","netanyahu","erdogan","pelosi","mcconnell","gop","democrat","republican",
    "labour","conservative","liberal","government","minister","president","prime minister",
    "parliament","congress","senate","white house","kremlin","pentagon","administration",
    "federal","cabinet","supreme court","legislation","constitution","chancellor",
    "election","vote","ballot","campaign","referendum","political","democracy","autocracy",
    "inauguration","midterm","polling","candidate","party","coalition","diplomatic",
    "diplomacy","sanctions","treaty","nato","united nations","g20","g7","embassy",
    "ambassador","foreign","bilateral","multilateral","summit","ceasefire","peace talks",
    "alliance","geopolitical","war","military","troops","army","navy","missile","nuclear",
    "weapons","airstrike","invasion","occupation","terrorist","insurgent","coup","regime",
    "conflict","border","sovereignty","annexation","casualties","economic","trade","tariff",
    "gdp","inflation","interest rate","federal reserve","central bank","stimulus","budget",
    "deficit","debt","policy","reform","subsidy","china","india","russia","iran","ukraine",
    "israel","gaza","taiwan","north korea","pakistan","afghanistan","iraq","syria","europe",
    "asia","middle east","official","spokesperson","statement","briefing","investigation",
    "hearing","testimony","indictment","impeachment","scandal","whistleblower","classified",
...
def section_title(text, subtitle=None):
    sub_html = f'<div style="font-size:clamp(11px,1.3vw,13px);color:#6e7f96;margin-top:4px;">{subtitle}</div>' if subtitle else ""
    st.markdown(f"""
...
def model_card(label, risk, prob):
    ...
    <div style="background:#0d1420;border:1px solid #1a2235;border-radius:14px;
        ...
        <div style="font-size:clamp(9px,1vw,11px);font-weight:600;color:#6e7f96;
                    text-transform:uppercase;letter-spacing:0.08em;
                    margin-bottom:12px;font-family:JetBrains Mono,monospace;">{label}</div>
    ...
    with st.sidebar:
        ...
        <div style="padding:20px 16px 16px;border-bottom:1px solid #1a2235;margin-bottom:16px;">
            ...
            <div style="font-size:12px;color:#6e7f96;line-height:1.7;
                ...
                An AI-powered system using four independent models — RoBERTa,
                Qwen2.5-3B (zero-shot), XGBoost, and Logistic Regression —
                to assess misinformation risk and explain which words drive
                the prediction.
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for level, cfg in RISK_CONFIG.items():
            rng = {"Low":"0–30%","Moderate":"30–60%","High":"60–85%","Critical":"85–100%"}[level]
            st.markdown(f"""
            <div style="display:flex;align-items:center;padding:7px 16px;
                ...
                <span style="font-size:13px;color:{cfg['text']};font-weight:500;flex:1;">{level}</span>
                <span style="font-size:11px;color:#3a5070;font-family:JetBrains Mono,monospace;">{rng}</span>
            </div>
            """, unsafe_allow_html=True)
...
def render_results(results, text_input, tfidf, fb):
    ...
    if not is_political:
        ...
        st.warning(
            ...
            "Qwen2.5 (zero-shot) is more robust to domain shift."
        )
